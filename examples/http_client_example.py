...
class MCPMathHTTPClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8000", enable_streaming: bool = True):
        ...
        if enable_streaming:
            ...
            print(f"📄 Streaming support: disabled")
    ...
    async def send_message(self, method: str, params: Dict[str, Any] = None, use_streaming: bool = None) -> Dict[str, Any]:
        ...
        if use_streaming is None:
            use_streaming = self.enable_streaming and self._should_stream(method, params)
        ...
        try:
            ...
            if use_streaming and "text/event-stream" in response.headers.get("content-type", ""):
    ...
    def _should_stream(self, method: str, params: Dict[str, Any]) -> bool:
        ...
        streaming_functions = [
            "fibonacci", "prime_factors", "next_prime", "collatz_sequence",
            "partition_count", "continued_fraction_expansion", "vampire_numbers"
        ]
    ...
    async def _handle_streaming_response(self, response) -> Dict[str, Any]:
            ...
            if line.startswith('data: '):
                try:
                    ...
                    if event_type == "start":
                        ...
                        print(f"  ✅ Stream completed in {exec_time:.3f}s ({events_received} events)")
        ...
        return final_result or {"error": {"code": -32603, "message": "No final result in stream"}}
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any], force_streaming: bool = None) -> Any:
...
async def start_http_server(port: int = 8000) -> subprocess.Popen:
    ...
    if not server_path:
        server_cmd = ["python", "-m", "chuk_mcp_math_server.math_server", "--transport", "http", "--port", str(port)]
    else:
        server_cmd = ["python", server_path, "--transport", "http", "--port", str(port)]
...
async def demonstrate_http_number_theory():
    ...
    try:
        ...
        try:
            ...
            print(f"📊 Functions available: {server_info.get('functions_available', 0)}")
        except Exception as e:
        ...
        try:
            print(f"🔍 Computing next_prime(10000) with forced streaming...")
            result = await client.call_tool("next_prime", {"n": 10000}, force_streaming=True)
...
async def demonstrate_http_arithmetic():
    ...
    try:
        ...
        try:
            ...
            print(f"💓 Server health: {health.get('status', 'unknown')} (score: {health.get('health_score', 0)})")
        except Exception as e:
...
async def test_http_server_connectivity(port: int = 8000):
    ...
    try:
        ...
        try:
            result = await client.call_tool("add", {"a": 7, "b": 13}, force_streaming=False)
